...
class trie {
    private:
        ...
        node_t *create_node(char index) {
            AO_fetch_and_add1(nodes_created);
    ...
    public:
        /* Trie constructor.
         *  Initialize root node and counters
         * */
        explicit trie() {
            memset(root_node.children, 0, sizeof(root_node.children));
            root.tag = NODE;
            root.n = &root_node;
            nodes_created = new int;
            *nodes_created = 0;
            buckets_created = 0;
        }
        /* True Destructor
         *  Do a recursive deletion of the nodes.
         * */
        ~trie() {
            delete_node(&root);
        ...
        /*
         * Search the trie by using the find_bucket method, then
         * calling find_in bucket to get the data out.
         * */
        V *search(K key) {
